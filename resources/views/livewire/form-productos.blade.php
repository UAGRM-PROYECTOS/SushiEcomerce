<div class="p-6 lg:p-8 bg-white border-b border-gray-200">
    ...
    <div class="flex flex-col justify-center">
        <form wire:submit.prevent='store'>
            ...
            <div class="mt-5">
                <x-label for="descuento_id" value="{{ __('Descuento') }}" />
                <select class="block mt-1 w-full rounded-lg" wire:model="descuento_id" id="descuento_id">
                    <option hidden selected>- SELECCIONE -</option>
                    @foreach ($descuentos as $descuento)
                    <option value="{{$descuento->id}}">{{$descuento->descuento}}</option>
                    @endforeach
                </select>
            </div>
